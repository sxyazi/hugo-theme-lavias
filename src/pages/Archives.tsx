...
const Line = ({archive}: { archive: Archive }) => {
  return (
    <article class="mb-4">
      ...
      <time class="ml-2 text-xs text-slate-500 dark:text-slate-400">{formatDate(archive.date, 'w, m d, Y')}</time>
    </article>
  )
}
  ...
  return (
    <Layout>
      ...
      {archives.map((archive) => {
        ...
        return (
          <>
            {newYear && <h2 class={`mt-10 mb-5 text-lg dark:text-slate-200`}>{archive.year}</h2>}
